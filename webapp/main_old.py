...
app = Flask(__name__)
app.register_blueprint(bp, url_prefix='/books')
...
@bp.route('/clear', defaults={'dir_path': ''})
@bp.route('/clear/<path:dir_path>')
@auth_check
...
@bp.route('/fuzz', methods=['GET', 'POST'])
@auth_check
def fuzzy_check():
    login_id = session['login_id']
